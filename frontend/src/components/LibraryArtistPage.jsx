...
export function LibraryArtistPage({ artistName, onBack }) {
  ...
  const searchTidalArtist = async (name, localContext) => {
    ...
    try {
      ...
      if (searchRes.items?.length > 0) {
        ...
        if (!tidalArtist) {
          console.warn(
            `No exact match for '${name}', using first result: ${items[0].name}`,
          );
          tidalArtist = items[0];
        }
  ...
  const mergeData = (local, remote) => {
    ...
    remoteAlbums.forEach((rAlbum) => {
      ...
      matchIndex = localAlbums.findIndex(
        (l, idx) =>
      ...
      <div class="flex items-start gap-6 mb-8">
        <div class="w-32 h-32 rounded-full overflow-hidden shadow-lg bg-surface-alt flex-shrink-0 border-4 border-surface">
          {remoteData?.artist.picture ? (
            <img
              src={api.getCoverUrl(remoteData.artist.picture, 320)}
              class="w-full h-full object-cover"
            />
          ) : localData.cover_path ? (
            <img
              src={api.getLocalCoverUrl(localData.cover_path)}
              class="w-full h-full object-cover"
            />
          ) : (
        ...
        <div class="flex-1 mt-2">
          <button
            onClick={onBack}
            class="mb-2 text-sm text-text-muted hover:text-text flex items-center gap-1 transition-colors"
          >
            <svg
              class="w-4 h-4"
              fill="none"
          ...
          <div class="flex items-center gap-3 text-sm text-text-muted">
            ...
            {searchingTidal && (
              <span class="flex items-center gap-1 text-primary">
                <span class="animate-spin">⟳</span> Syncing with Tidal...
              </span>
            )}
          </div>
        </div>
              ...
              >
                <div class="aspect-square mb-3 rounded-md overflow-hidden bg-surface-alt shadow-sm relative">
                  ...
                  {album.cover ? (
                    ...
                    <img
                      src={api.getLocalCoverUrl(album.cover_path)}
                      class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
                      loading="lazy"
                    />
                  ) : (
                  ...
                  <div class="absolute top-2 right-2 flex flex-col gap-1 items-end">
                    {album.origin === "merged" &&
                      album.remoteTracks > album.tracks.length && (
                ...
                <h3
                  class="font-bold text-sm text-text truncate group-hover:text-primary transition-colors"
                  title={album.title}
                >
                  {album.title}
                </h3>
                <p class="text-xs text-text-muted flex items-center gap-1.5">
                  ...
                  {album.origin === "merged" &&
                    album.remoteTracks > album.tracks.length && (
                      <span class="text-yellow-600 dark:text-yellow-400 text-[10px]">
                        • Incomplete
                      </span>
                    )}
                </p>
        ...
        <section>
          <div class="flex items-center justify-between mb-4 px-2 pt-4 border-t border-border/50">
            ...
            <span class="text-sm text-text-muted">
              {mergedAlbums.filter((a) => a.origin === "remote").length} albums
            </span>
          </div>
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {mergedAlbums
...
const UnifiedAlbumModal = ({
    ...
    const loadContent = async () => {
      ...
      try {
        ...
        const mergedTracks = remoteItems.map((rt) => {
          const localMatch = localTracks.find(
            (lt) => normalize(lt.title) === normalize(rt.title),
          );
          return {
            ...rt,
            isLocal: !!localMatch,
  ...
  const downloadCount = tracks.filter(
    (t) => selectedIds.has(t.id) && !t.isLocal,
  ).length;
        ...
        <div class="p-6 border-b border-border flex gap-6 items-start bg-surface-alt/30">
          <img
            src={
              album.cover
                ...
                : album.cover_path
                  ? api.getLocalCoverUrl(album.cover_path)
                  : null
            }
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-text-muted mb-2">
              {artistName} • {tracks.length} Tracks • {album.year}
            </p>

            <div class="flex gap-2">
              {album.origin === "remote" && (
                <span class="badge bg-primary/10 text-primary">On Tidal</span>
              )}
              {album.origin === "local" && (
                <span class="badge bg-surface text-text-muted border border-border">
                  Local Only
                </span>
              )}
              {album.origin === "merged" && (
                <span class="badge bg-green-500/10 text-green-600 border border-green-500/20">
                  Library + Tidal
                </span>
              )}
            </div>
            ...
            {missingCount > 0 && (
              <button
                ...
                class="btn-primary mt-2 text-xs self-start flex items-center gap-2"
              >
                <svg
                  class="w-4 h-4"
                  fill="none"
          ...
          <button
            onClick={onClose}
            class="self-start p-2 rounded-full hover:bg-surface-alt transition-colors"
          >
            <svg
          ...
          {loading ? (
            ...
            <div class="flex flex-col gap-1">
              {tracks.map((track, i) => (
                ...
                >
                  <div class="w-8 text-center text-sm text-text-muted">
                    {track.trackNumber}
                  </div>
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-text-muted flex items-center gap-2">
                      ...
                      {track.audioQuality === "HI_RES" && (
                        <span class="text-[10px] bg-yellow-500/10 text-yellow-500 px-1 rounded">
                          MQA
                        </span>
                      )}
                      {track.audioQuality === "LOSSLESS" && (
                        <span class="text-[10px] bg-cyan-500/10 text-cyan-500 px-1 rounded">
                          HiFi
                        </span>
                      )}
