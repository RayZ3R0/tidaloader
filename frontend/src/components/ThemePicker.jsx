  ...
  try {
    if (typeof sessionStorage !== "undefined") {
      ...
      window.dispatchEvent(
        new CustomEvent("nyanCatStateChange", {
          detail: { enabled },
        }),
      );
    }
...
export function ThemePicker() {
  ...
  const groupedThemes = () => {
    ...
    const remainingThemes = filteredThemes.filter(
      ([themeKey]) => !recentThemes.includes(themeKey),
    );
            ...
            >
              ...
              <div class="max-h-[min(420px,70vh)] overflow-y-auto py-1 thin-scrollbar">
                {groupedThemes().map(([themeKey, themeName], index) => {
                  const isRecent =
                    !searchQuery && recentThemes.includes(themeKey);
                  const isFirstNonRecent =
                    !searchQuery &&
                    index === recentThemes.length &&
                  ...
                  return (
                    ...
                    <div key={themeKey}>
                      ...
                      ${theme === themeKey ? "border-primary shadow-sm" : "border-border"}`}
                        >
                          {themePreviewColors[themeKey] &&
                            themePreviewColors[themeKey].map((color, i) => (
                              <div
                                key={i}
                                class="w-full h-full"
                                style={{ backgroundColor: color }}
                ...
                <div class="flex items-center justify-between text-xs">
                  <p class="text-text-muted flex items-center gap-1.5">
                    ...
                    >
                      ...
                      {/* Tooltip for NyanCat */}
                      <span
                        class="absolute left-1/2 -bottom-1 transform -translate-x-1/2 translate-y-full 
                  ...
                  <span class="text-text-muted">
                    {Object.keys(themePreviewColors).length} themes
                  </span>
                </div>
              </div>
            </div>
