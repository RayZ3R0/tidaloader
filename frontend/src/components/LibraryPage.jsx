      ...
      {loading && artists.length === 0 ? (
        ...
        <div class="text-center py-24 bg-surface-alt/30 rounded-xl border border-border-light border-dashed">
          ...
          <p class="text-text-muted max-w-sm mx-auto mb-6">
            No music files found in your local directory. Download some music
            from the Search tab to populate your library.
          </p>
        </div>
      ) : (
...
function ArtistCard({ artist, onClick }) {
  ...
  useEffect(() => {
    ...
    if (!picture && !artist.image && artist.tidal_id) {
      ...
      api
        .getArtist(artist.tidal_id)
        .then((details) => {
          if (details.artist?.picture) {
            setPicture(details.artist.picture);
            // Persist to backend cache
            api
              .updateLibraryArtist(artist.name, {
                picture: details.artist.picture,
              })
              .catch((e) => console.warn("Failed to cache artist picture", e));
          }
        })
        .catch((err) =>
          console.debug(`Failed to fetch picture for ${artist.name}`, err),
        )
  ...
  const imageUrl = picture
    ...
    : artist.image
      ? api.getLocalCoverUrl(artist.image)
      : null;
