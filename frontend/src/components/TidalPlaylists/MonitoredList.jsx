...
export function MonitoredList() {
  ...
  const fetchPlaylists = async () => {
    try {
      ...
      const tidalPlaylists = res.filter(
        (p) => !p.source || p.source === "tidal",
      );
  ...
  const handleRemove = async (uuid) => {
    if (
      !confirm(
        "Are you sure you want to stop monitoring this playlist? Files won't be deleted.",
      )
    )
      return;
    try {
    ...
    return (
      <div class="text-center p-12 text-text-muted bg-surface-alt rounded-lg border border-dashed border-border">
        ...
        <p class="text-sm mt-2">
          Go to the "Search & Add" tab to start syncing a playlist.
        </p>
      </div>
    );
  }
          ...
          {playlists.map((playlist) => (
            ...
            >
              ...
              <td class="p-4 capitalize">
                {editingId === playlist.uuid ? (
                  ...
                  <button
                    ...
                    class={`inline-block px-2 py-1 text-xs rounded-full cursor-pointer hover:scale-105 transition-transform ${
                      playlist.sync_frequency === "manual"
              ...
              <td class="p-4 text-sm text-text-muted">
                {playlist.last_sync
                  ? new Date(playlist.last_sync).toLocaleString()
                  : "Never"}
              </td>
